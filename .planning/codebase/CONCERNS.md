...
- Risk: `backend/plumber.R:60-65` sets `Access-Control-Allow-Origin: *` for every route (the `#filter cors` runs before the auth filter). Combined with Bearer-token auth (`Authorization` header, not a cookie), a wildcard alone does not let a foreign page silently attach a victim's token — the calling JS still has to set the header itself — but it does mean any origin can read the response to a request it CAN authenticate (e.g. one made using a token obtained by other means, or public/anonymous-readable data), and it removes any origin-based defense-in-depth. Issue #4 ("Restrict CORS in production") was closed with no visible corresponding change to this filter.
...
- Recommendations: restrict `Access-Control-Allow-Origin` to the known frontend origin(s) (env-configurable for dev vs. prod), especially since `/health`, `/auth/login`, `/auth/register` are public and unauthenticated.
